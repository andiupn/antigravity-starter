...
### Part A: Agent Config Parity

1. **Discover agent files** — Glob `.gemini/agents/*.md` untuk dapatkan semua agent definitions
2. **Parse frontmatter** — Read setiap file, extract `name`, `tools`, `model` dari YAML frontmatter (antara `---` pertama dan `---` kedua)
3. **Read AGENTS.md inventory** — Read `.gemini/AGENTS.md`, locate tabel "Daftar Agen", parse baris per agent (kolom: name, tools, model)
4. **Compare per-agent:**
...
### Part B: Skill Registry Parity

5b. **Discover skill files** — `find .gemini/skills -maxdepth 2 -name SKILL.md`
6b. **Parse frontmatter** tiap skill — extract `name:` menggunakan awk:
    ```bash
    awk '/^---/{c++; if(c==2) exit} c==1 && /^name:/{print $2}' <file>
    ```
7b. **Read SKILLS.md** inventory — parse kolom pertama tabel
8b. **Compare:**
    - ⚠️ WARN: skill punya SKILL.md tapi tidak terdaftar di SKILLS.md
    - ❌ FAIL: skill terdaftar di SKILLS.md tapi tidak ada direktori/SKILL.md
...
| Skill | SKILL.md | SKILLS.md | Status |
|---|---|---|---|
| git-workflow | ✅ | ✅ | ✅ sync |
...
- Model comparison case-insensitive
- Tools comparison: normalize ke sorted list sebelum compare (order tidak penting)
- Jika AGENTS.md tidak ada section daftar agen: report sebagai error konfigurasi
