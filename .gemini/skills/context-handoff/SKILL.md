...
description: |
  Simpan ringkasan context session saat ini ke .gemini/memory/ sebelum /compact atau akhir session,
...
**Bahasa output:** Indonesia. Gunakan English untuk istilah teknis (file path, function name, command).

## Tujuan

Buat snapshot ringkas dari session saat ini dan simpan ke `.gemini/memory/` sebelum context di-reset via `/compact`. Dengan ini, session berikutnya punya cukup context untuk melanjutkan tanpa perlu recap panjang.
...
Read `.gemini/memory/MEMORY.md` untuk tahu apa yang sudah pernah disimpan sebelumnya.
...
Buat file baru: `.gemini/memory/handoff-YYYY-MM-DD-HH.md`
...
Append ke `.gemini/memory/MEMORY.md` di bagian atas (reverse-chrono):
...
File: .gemini/memory/handoff-YYYY-MM-DD-HH.md
MEMORY.md: updated
